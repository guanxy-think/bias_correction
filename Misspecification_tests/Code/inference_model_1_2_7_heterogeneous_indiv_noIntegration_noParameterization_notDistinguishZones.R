...
for (a_file in file_names) {
  ...
  pop_densities_unique <- pop_densities_unique[10]    #consider only zone 10
  ...
  obs_12_index <- which((data_baseline$capture_1_new==1 & data_baseline$capture_2_new==1) & data_baseline$trip_density_new==pop_densities_unique)
  obs_1_index <- which((data_baseline$capture_1_new==1 & data_baseline$capture_2_new==0) & data_baseline$trip_density_new==pop_densities_unique)
  obs_2_index <- which((data_baseline$capture_1_new==0 & data_baseline$capture_2_new==1) & data_baseline$trip_density_new==pop_densities_unique)
  ...
  likelihood_function <- function(par) {
    ...
    inf_count_trip <- length(which(p_trips_1 >= 1-1e-20)) + length(which(p_trips_2 >= 1-1e-20)) + length(which(p_trips_1 <= 1e-20)) + length(which(p_trips_2 <= 1e-20))
    # p_trips_1[which(p_trips_1 >= 1-1e-20)] <- 1-1e-10
    # p_trips_2[which(p_trips_2 >= 1-1e-20)] <- 1-1e-10
    # p_trips_1[which(p_trips_1 <= 1e-20)] <- 1e-10
    # p_trips_2[which(p_trips_2 <= 1e-20)] <- 1e-10
    
    LL_indiv_12 <- log(p_trips_1[obs_12_index]) + log(p_trips_2[obs_12_index])
    LL_indiv_1 <- log(p_trips_1[obs_1_index]) + log(1-p_trips_2[obs_1_index])
    LL_indiv_2 <- log(1-p_trips_1[obs_2_index]) + log(p_trips_2[obs_2_index])
    LL_indiv_12[which(is.infinite(LL_indiv_12))] <- -500
    LL_indiv_1[which(is.infinite(LL_indiv_1))] <- -500
    LL_indiv_2[which(is.infinite(LL_indiv_2))] <- -500
    ...
    inf_count <- length(which(p_zones_1 >= 1-1e-20)) + length(which(p_zones_2 >= 1-1e-20)) + length(which(p_zones_1 <= 1e-20)) + length(which(p_zones_2 <= 1e-20))
    # p_zones_1[which(p_zones_1 >= 1-1e-20)] <- 1-1e-10
    # p_zones_2[which(p_zones_2 >= 1-1e-20)] <- 1-1e-10
    # p_zones_1[which(p_zones_1 <= 1e-20)] <- 1e-10
    # p_zones_2[which(p_zones_2 <= 1e-20)] <- 1e-10
    
    LL_zone_1 <- obs_1_zones_count*(log(p_zones_1) + log(1-p_zones_2))
    LL_zone_2 <- obs_2_zones_count*(log(1-p_zones_1) + log(p_zones_2))
    LL_zone_12 <- obs_12_zones_count*(log(p_zones_1) + log(p_zones_2))
    LL_zone_12[which(is.infinite(LL_zone_12))] <- -500
    LL_zone_1[which(is.infinite(LL_zone_1))] <- -500
    LL_zone_2[which(is.infinite(LL_zone_2))] <- -500
    ...
    cat(par,file = "C:/Users/guanxy/Documents/logfile_server_17.txt", append=T,fill=T)
    cat(lambda_a,file = "C:/Users/guanxy/Documents/logfile_server_17.txt", append=T,fill=T)
    cat(inf_count+inf_count_trip,file = "C:/Users/guanxy/Documents/logfile_server_17.txt", append=T,fill=T)
    ...
    LL_0_infer[which(is.infinite(LL_0_infer))] <- -500
    combination_term_log <- lfactorial(lambda_a) - lfactorial(obs_0_zones_count) #- lfactorial(obs_1_zones_count) - lfactorial(obs_2_zones_count) - lfactorial(obs_12_zones_count)#lfactorial(obs_zones_counts)#
    
    #LL_0_infer[which(is.infinite(LL_0_infer))] <- -200000
    
    log_likelihood_output <- sum(combination_term_log) + LL_0_infer + sum(LL_zone_12) + sum(LL_zone_1) + sum(LL_zone_2)
    
    #if (is.infinite(log_likelihood)) log_likelihood <- -100000
    
    cat(log_likelihood_output,file = "C:/Users/guanxy/Documents/logfile_server_17.txt", append=T,fill=T)
  ...
  para_init <- c(fit_1_lm$coefficients, fit_2_lm$coefficients)
  
  # estim_optim <- spg(para_init,likelihood_function,lower=c(rep(-Inf,6),obs_zones_counts),upper = c(rep(Inf,7)),
  #              control = list(maxit = 300000, trace = TRUE, maxfeval=500000))    #, eps=0.0000001
  
  L <- c()
  for (a in obs_zones_counts:20000) {
    estim_optim <- optim(para_init,likelihood_function, method="L-BFGS-B", lower = c(rep(-Inf,6)), upper = c(rep(Inf,6)), #hessian=T,
                         control = list(maxit = 30000,trace = TRUE,ndeps=c(rep(0.0000001,6)),factr=1e5,REPORT=1))
    L <- c(L,estim_optim$value)
  }
...
###test likelihood
plot(obs_zones_counts:(obs_zones_counts+6600), -L[1:6601], xlab="N", ylab="Log-likelihood",ylim=c(1790, 1810))

LL_1 <- c()
para <- c(fit_1_lm$coefficients, fit_2_lm$coefficients, ground_truth)
for (para_8 in 0:10000) {
  para[8] <- para_8
  ll <- likelihood_function(para)
  LL_1 <- c(LL_1, ll)
}
plot(1:5000,  LL_1[1:5000], type="l", xlab="N_2", ylab="Log-likelihood")

LL_2 <- c()
para <- c(fit_1_lm$coefficients, fit_2_lm$coefficients, ground_truth)
for (para_16 in 402:10000) {
  para[16] <- para_16
  ll <- likelihood_function(para)
  LL_2 <- c(LL_2, ll)
}
plot(402:10000,  LL_2, type="l", xlab="N_10", ylab="Log-likelihood")
