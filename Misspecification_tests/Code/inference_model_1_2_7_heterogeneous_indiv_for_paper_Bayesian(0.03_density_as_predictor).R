...
file_names <- c("scenario2(-0.03).csv")
                #"SR_LU","SU_LU","LU09_LU"
...
for (a_file in file_names) {
  ...
  likelihood_function <- function(par) {
    ...
    #cat(par,file = "C:/Users/guanxy/Documents/logfile_paper_0.03_density_lambda10_MAP.txt", append=T,fill=T)
    #cat(lambda_a,file = "C:/Users/guanxy/Documents/logfile_paper_0.03_density_lambda10_MAP.txt", append=T,fill=T)
    ...
    #cat(log_likelihood_output,file = "C:/Users/guanxy/Documents/logfile_paper_0.03_density_lambda10_MAP.txt", append=T,fill=T)
    
    ridged_LL <- -log_likelihood_output - sum(dnorm(par,avg_prior,std_err_prior,log=T))#+ 10*sum(par[c(2:3,5:6)]^2)    #########
  ...
  gibbs_samples[1,1:3] <- fit_1_lm_obs$coefficients
  gibbs_samples[1,4:6] <- fit_2_lm_obs$coefficients
  gibbs_samples[1,7:9] <- fit_lm_obs$coefficients
  ...
  for (m in 2:2000) {
    ...
    write.csv(gibbs_samples,"G:/My Drive/trip bias paper/scenarios/scenario_2_and_3/predictor_correlation/scenario2(-0.03)_density_results_MAP.csv",row.names = F)
  }
