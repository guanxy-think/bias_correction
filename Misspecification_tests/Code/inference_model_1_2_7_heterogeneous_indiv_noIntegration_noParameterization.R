...
for (a_file in file_names) {
  ...
  pop_densities_unique <- pop_densities_unique[10]    #remove the second zone
  ...
  likelihood_function <- function(par) {
    ...
    # p_trips_1[which(p_trips_1 > 1-1e-20)] <- 1-1e-20
    # p_trips_2[which(p_trips_2 > 1-1e-20)] <- 1-1e-20
    # p_trips_1[which(p_trips_1 < 1e-20)] <- 1e-20
    # p_trips_2[which(p_trips_2 < 1e-20)] <- 1e-20
    ...
    LL_1_for_each_zone[which(is.infinite(LL_1_for_each_zone))] <- -2000
    LL_2_for_each_zone[which(is.infinite(LL_2_for_each_zone))] <- -2000
    LL_12_for_each_zone[which(is.infinite(LL_12_for_each_zone))] <- -2000
    ...
    # p_zones_1[which(p_zones_1 > 1-1e-20)] <- 1-1e-20
    # p_zones_2[which(p_zones_2 > 1-1e-20)] <- 1-1e-20
    # p_zones_1[which(p_zones_1 < 1e-20)] <- 1e-20
    # p_zones_2[which(p_zones_2 < 1e-20)] <- 1e-20
    ...
    combination_term_log <- lfactorial(lambda_a) - lfactorial(obs_0_zones_count*(1+a)) #- lfactorial(obs_1_zones_count) - lfactorial(obs_2_zones_count) - lfactorial(obs_12_zones_count)#lfactorial(obs_zones_counts)#
    ...
    log_likelihood_output <- sum(combination_term_log) + sum(LL_1_for_each_zone) + sum(LL_2_for_each_zone) + sum(LL_12_for_each_zone) + sum(LL_0_infer)
  ...
  para_init <- c(fit_1_lm$coefficients, fit_2_lm$coefficients, obs_zones_counts+10)
  
  # estim_optim <- spg(para_init,likelihood_function,lower=c(rep(-Inf,6),obs_zones_counts),upper = c(rep(Inf,16)),
  #              control = list(maxit = 300000, trace = TRUE, maxfeval=500000))    #, eps=0.0000001
  b <- c()
  L <- c()
  for (a in seq(0.0001,0.002,0.0001)) {
    estim_optim <- optim(para_init,likelihood_function, method="L-BFGS-B", lower = c(rep(-Inf,6),obs_zones_counts), upper = rep(Inf,7), #hessian=T,
                         control = list(maxit = 30000,trace = TRUE,ndeps=c(rep(0.0000001,6),rep(0.1,1)),factr=1e5,REPORT=1))
    b <- c(b,estim_optim$par[7])
