...
for (a_file in file_names) {
  result_path <- paste("G:/My Drive/trip bias paper/scenarios/results/pop_density/",a_file,".csv",sep="")
  ...
  para_std <- para_info$se_data*sqrt(obs_all_count)
  ...
  par_1_all <- para_info$para_data[1] #rnorm(10000, para_info$para_data[1],para_std[1])
  par_2_all <- para_info$para_data[2] #rnorm(10000, para_info$para_data[2],para_std[2])
  par_3_all <- para_info$para_data[3] #rtruncnorm(10000, 0, Inf, para_info$para_data[3],para_std[3])
...
write.csv(results,"G:/My Drive/trip bias paper/scenarios/results/n_a_interval.csv",row.names=F)
