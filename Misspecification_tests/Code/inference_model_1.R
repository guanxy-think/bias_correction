...
for (a_file in file_names) {
  ...
  likelihood_function <- function(par) {
    ...
    poisson_numbers <- t(sapply(1:100, function(x) rpois(5,par[5:9])))
    
    cat(par,file = "G:/My Drive/trip bias paper/scenarios/results/logfile_server.txt", append=T,fill=T)
    all_likeluhoods <- foreach(j = 1:100, .combine="rbind", .export=c("obs_zones_counts","obs_1_zones_count","obs_2_zones_count","obs_12_zones_count")) %dopar% {
      n_a_zones <- poisson_numbers[j,]
      ...
      n_a_zones <- obs_0_zones_count + obs_1_zones_count + obs_2_zones_count + obs_12_zones_count
  ...
  estim <- spg(c(-2,0.01,-2,0.01,rep(1000,5)),likelihood_function,lower=c(rep(-Inf,4),rep(0,5)),control = list(maxit = 300000, trace = TRUE, maxfeval=500000))
  # estim <- optim(c(-2,0.01,-2,0.01,rep(1000,5)),likelihood_function, method="L-BFGS-B", lower = c(rep(-Inf,4),rep(0,5)), upper = Inf, hessian=T,
  #              control = list(maxit = 30000,trace = TRUE,ndeps=rep(0.0000000001,9),factr=0.45e9,REPORT=1))
  para_data <- estim$par
