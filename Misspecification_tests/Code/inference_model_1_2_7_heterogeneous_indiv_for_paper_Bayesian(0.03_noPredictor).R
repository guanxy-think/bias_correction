...
for (a_file in file_names) {
  ...
  likelihood_function <- function(par) {
    ...
    cat(par,file = "C:/Users/guanxy/Documents/logfile_paper_0.03_noPredictor_lambda10_MAP.txt", append=T,fill=T)
    cat(lambda_a,file = "C:/Users/guanxy/Documents/logfile_paper_0.03_noPredictor_lambda10_MAP.txt", append=T,fill=T)
    # cat(inf_count,file = "C:/Users/guanxy/Documents/logfile_server_9.txt", append=T,fill=T)
    
    if (max(poisson_upper_bound) > 30000) return(30000)#max(poisson_upper_bound))    #######
    if (max(lambda_a < obs_zones_counts) == 1) return(30000) #Inf)#                   #######
    ...
    cat(log_likelihood_output,file = "C:/Users/guanxy/Documents/logfile_paper_0.03_noPredictor_lambda10_MAP.txt", append=T,fill=T)
    
    ridged_LL <- -log_likelihood_output - sum(dnorm(par,avg,std_err,log=T))#+ 10*sum(par[c(2:3,5:6)]^2)    #########
  ...
  write.csv(results,"G:/My Drive/trip bias paper/scenarios/scenario_2_and_3/predictor_correlation/regularization/scenario2(-0.03)_noPredictor_MAP.csv",row.names=F)
}
