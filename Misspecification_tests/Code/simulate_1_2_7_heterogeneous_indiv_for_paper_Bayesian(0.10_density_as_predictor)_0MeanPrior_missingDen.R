...
result <- read.csv("G:/My Drive/trip bias paper/scenarios/scenario_2_and_3/predictor_correlation/scenario2(-0.10)_density_negNumDensityCor_results_MAP_0MeanPrior_noConst_indiv.csv")
par <- colMeans(result[1000:2000,])

file_names <- c("scenario2(-0.10)_negNumDensityCor.csv")
                #"SR_LU","SU_LU","LU09_LU"
...
for (a_file in file_names) {
  ...
  fit_lm_obs <- lm(log(prior) ~ income_zone)
  fit_1_lm_obs <- lm(obs_1_zones_count_N_mix/obs_zones_counts ~ income_zone) #glm(data_baseline$capture_1_new~data_baseline$trip_density+data_baseline$trip_income,family=binomial)
  fit_2_lm_obs <- lm(obs_2_zones_count_N_mix/obs_zones_counts ~ income_zone)
  ...
  prior_1[prior_1 == 0] <- 1
  fit_lm_obs_1 <- lm(log(prior_1) ~ income_zone)
  ...
  likelihood_function <- function(par) {
    ...
    utility_zones_1 <- par[1] + par[2]*income_zone
    utility_zones_2 <- par[3] + par[4]*income_zone
    ...
    lambda_a <- exp(par[5] + par[6]*income_zone)  #obs_zones_counts/(p_zones_1 + p_zones_2 - p_zones_1*p_zones_2)#rpois(1,lambda_a)
    ...
    ridged_LL <- -log_likelihood_output - sum(dnorm(par[c(1,2,3,4)],0,std_err_prior[c(1,2,3,4)],log=T))#+ 10*sum(par[c(2:3,5:6)]^2)    #########
  ...
  plot(pop_densities_unique, lambda_a*100/ground_truth, xlab="Percentage of observed trips over ground truth", ylab="Percentage of inferred trips over ground truth")
  abline(h=100,col="red",lty=2)
  output <- cbind(obs_zones_counts*100/ground_truth, lambda_a*100/ground_truth)
  write.csv(output, "G:/My Drive/trip bias paper/scenarios/scenario_2_and_3/predictor_correlation/LLs/percentages_0.10_den_missing.csv",row.names=F)
  ...
  set.seed(6)
  ### Gibbs sampling
  gibbs_samples <- matrix(NA,3000,6)    #alpha_1, alpha_2, error_1, error_2, n_a
  ...
  sds <- std_err #abs(c(summary(fit_1_lm)$coefficients[,2],summary(fit_1_lm)$coefficients[,2],summary(fit_lm)$coefficients[,2]))

  for (m in 2:2000) {
    #clusterExport(cl, "i")

    for (n in 1:6) {
      para <- matrix(NA,100,6)
      para[,1:n] <- matrix(gibbs_samples[m,1:n],nrow=100,ncol=n,byrow=T)
      para[,n:6] <- matrix(gibbs_samples[m-1,n:6],nrow=100,ncol=7-n,byrow=T)
    ...
    write.csv(gibbs_samples,"G:/My Drive/trip bias paper/scenarios/scenario_2_and_3/predictor_correlation/scenario2(-0.10)_density_negNumDensityCor_results_MAP_0MeanPrior_missingDen.csv",row.names = F)
  }
