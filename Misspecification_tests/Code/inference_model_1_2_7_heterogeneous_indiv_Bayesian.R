...
for (a_file in file_names) {
  ...
  likelihood_function <- function(par) {
    ...
    cat(par,file = "C:/Users/guanxy/Documents/logfile_server_10_reject_sampling.txt", append=T,fill=T)
    cat(lambda_a,file = "C:/Users/guanxy/Documents/logfile_server_10_reject_sampling.txt", append=T,fill=T)
    # cat(inf_count,file = "C:/Users/guanxy/Documents/logfile_server_10.txt", append=T,fill=T)
    ...
    for (i in 1:10) {
      likelihoods <- foreach(j = obs_zones_counts[i]:poisson_upper_bound[i], .combine="c", .export=c("obs_zones_counts","obs_1_zones_count","obs_2_zones_count","obs_12_zones_count")) %dopar% {
        ...
        LL_1 <- LL_1_for_each_zone[i] #obs_1_zones_count[i]*(log(p_zones_1[i]) + log(1 - p_zones_2[i]))
        LL_2 <- LL_2_for_each_zone[i] #obs_2_zones_count[i]*(log(1-p_zones_1[i]) + log(p_zones_2[i]))
        LL_12 <- LL_12_for_each_zone[i] #obs_12_zones_count[i]*(log(p_zones_1[i]) + log(p_zones_2[i]))
        ...
        log_likelihood_zones + dpois(j,lambda_a[i], log=T) #* dpois(j,lambda_a[i]) 
      }
    ...
    cat(log_likelihood_output,file = "C:/Users/guanxy/Documents/logfile_server_10_reject_sampling.txt", append=T,fill=T)
