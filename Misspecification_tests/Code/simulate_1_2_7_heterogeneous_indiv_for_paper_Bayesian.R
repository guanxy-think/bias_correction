...
result <- read.csv("G:/My Drive/trip bias paper/scenarios/scenario_2_and_3/predictor_correlation/scenario2(-0.03)_noPredictor_results.csv")

par <- colMeans(result[400:500,])
all_par <- matrix(NA,1900,9)
for (i in 1:1900) {
  all_par[i,] <- colMeans(result[i:(i+99),])
}

file_names <- c("scenario2(-0.03)_noPredictor.csv")
                #"SR_LU","SU_LU","LU09_LU"
...
for (a_file in file_names) {
  ...
  fit_lm <- lm(log(obs_zones_counts+1000) ~ pop_densities_unique +income_zone)
  fit_1_lm <- glm(data_baseline$capture_1[obs_index] ~ data_baseline$trip_density[obs_index] + data_baseline$trip_income[obs_index],family=binomial) #glm(data_baseline$capture_1_new~data_baseline$trip_density+data_baseline$trip_income,family=binomial)
  fit_2_lm <- glm(data_baseline$capture_2[obs_index] ~ data_baseline$trip_density[obs_index] + data_baseline$trip_income[obs_index],family=binomial)
  ...
  likelihood_function <- function(par) {
    ...
    cat(par,file = "C:/Users/guanxy/Documents/logfile_paper2.txt", append=T,fill=T)
    cat(lambda_a,file = "C:/Users/guanxy/Documents/logfile_paper2.txt", append=T,fill=T)
    ...
    cat(log_likelihood_output,file = "C:/Users/guanxy/Documents/logfile_paper2.txt", append=T,fill=T)
  ...
  plot(obs_zones_counts*100/ground_truth, lambda_a*100/ground_truth, xlab="Percentage of observed trips", ylab="Percentage of inferred trips")
