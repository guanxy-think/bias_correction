...
#for (a_file in file_names) {
  ...
  likelihood_function <- function(par) {
    ...
    utility_zones_1 <- par[1] + par[2]*income_zone + par[3]*pop_densities_unique + par[4]*population_unique_norm + par[5]*renter_HH_norm + par[6]*owner_HH_norm + par[7]*empoyment_norm
    p_zones_1 <- 1 - 1/(1+exp(utility_zones_1))
    utility_zones_2_given1 <- par[8] + par[9]*income_zone +  + par[10]*pop_densities_unique + par[11]*population_unique_norm + par[12]*renter_HH_norm + par[13]*owner_HH_norm + par[14]*empoyment_norm + par[22]*p_zones_1
    utility_zones_2_givenNot1 <- par[8] + par[9]*income_zone +  + par[10]*pop_densities_unique + par[11]*population_unique_norm + par[12]*renter_HH_norm + par[13]*owner_HH_norm + par[14]*empoyment_norm - par[22]*(1-p_zones_1)
    ...
    lambda_a <- exp(par[15] + par[16]*income_zone +  + par[17]*pop_densities_unique + par[18]*population_unique_norm + par[19]*renter_HH_norm + par[20]*owner_HH_norm + par[21]*empoyment_norm)  #obs_zones_counts/(p_zones_1 + p_zones_2 - p_zones_1*p_zones_2)#rpois(1,lambda_a)
    ...
    if (max(poisson_upper_bound) > 16000) return(Inf)#max(poisson_upper_bound))    #######
    if (max(lambda_a < obs_zones_counts) == 1) return(Inf)#30000)                    #######

    #all_likelihoods <- c()
    all_likelihoods <- foreach(i=1:12, .combine="c", .export=c("obs_zones_counts","obs_1_zones_count","obs_2_zones_count","obs_12_zones_count")) %dopar% {
    #for (i in 1:20) {     ##############
      likelihoods <- c()
  ...
  set.seed(6)
  gibbs_samples <- matrix(NA,4000,22)
  ...
  for (m in 2:4000) {
    for (n in 1:22) {
      ...
      para[n:22] <- gibbs_samples[m-1,n:22]
      para[n] <- proposal
      if (n == 1) {
    ...
    write.csv(gibbs_samples,"C:/Users/USXG713181/OneDrive/Google drive/trip bias paper/scenarios/real_world/real_world_results_reduced.csv",row.names = F)
  }
