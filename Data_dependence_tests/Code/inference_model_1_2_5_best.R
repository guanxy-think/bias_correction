...
for (a_file in file_names) {
  ...
  estim_optim <- spg(c(fit_1_lm$coefficients, fit_2_lm$coefficients, fit_lm$coefficients),likelihood_function,lower=c(rep(-Inf,5),0),upper = c(rep(Inf,5),0.01),
               control = list(maxit = 300000, trace = TRUE, maxfeval=500000))
  # estim_potim <- optim(c(fit_1_lm$coefficients, fit_2_lm$coefficients, fit_lm$coefficients),likelihood_function, method="L-BFGS-B", lower = c(rep(-Inf,5),0), upper = c(rep(Inf,5),0.01), #hessian=T,
  #                      control = list(maxit = 30000,trace = TRUE,ndeps=c(rep(0.000000001,6)),factr=1e5,REPORT=1))
  ...
  write.csv(results,paste("G:/My Drive/trip bias paper/scenarios/representativeness/inference_results/",a_file,"_best(32.69718).csv",sep=""),row.names=F)
}
