...
for (a_file in file_names) {
  result_path <- paste("G:/My Drive/trip bias paper/scenarios/representativeness/more_zones/inference_results/",a_file,"_indiv_model_multinom_indiv_income_indiv.csv",sep="")
  para_info <- read.csv(result_path)
  para_info$se_data[which(is.na(para_info$se_data))] <- 0
  
  # ### For bayesian estimation
  # para_values <- read.csv("G:/My Drive/trip bias paper/scenarios/representativeness/inference_results/gibbs_samples_1_multinomial.csv")
  # plot.ts(para_values$V4[1:800] - min(para_values$V4),xlim=c(0,800), xlab="Iterations", ylab="Likelihood")
  # para_info <- data.frame(para_data=numeric(6), se_data=numeric(6))
  # para_info$para_data <- colMeans(para_values[600:1000,])
  # para_info$se_data <- apply(para_values[600:1000,],2,sd)
  
  data_path <- paste('G:/My Drive/trip bias paper/scenarios/representativeness/more_zones/',a_file,"_regen_with_income.csv",sep="")
  ...
  ### using pop_density
  pop_densities_unique <- unique(data_info$density_trips)
  obs_zones_counts <- sapply(pop_densities_unique, function(x) length(which((data_info$capture_1==1 | data_info$capture_2==1) & data_info$density_trips==x)))
  truth_zone_counts <- sapply(pop_densities_unique, function(x) length(which(data_info$density_trips==x)))
  ...
  for (i in 1:3) {
    ...
    n_a_zones_from_lambda <- exp(para_set[5] + para_set[6]*pop_densities_unique)
...
for (a_file in file_names) {
  ...
  obs_den_1 <- data_info$density_trips[which(data_info$capture_1 == 1)]
  obs_den_2 <- data_info$density_trips[which(data_info$capture_2 == 1)]
  missing_den <-  data_info$density_trips[which(data_info$capture_2 == 0 & data_info$capture_1 == 0)]
  den_all <- data_info$density_trips
  ...
  #pop_densities_unique <- unique(data_info$density_trips)
  obs_zones_counts_1 <- sapply(pop_densities_unique, function(x) length(which(data_info$capture_1==1 & data_info$density_trips==x)))
  obs_zones_counts_2 <- sapply(pop_densities_unique, function(x) length(which(data_info$capture_2==1 & data_info$density_trips==x)))
  obs_zones_counts_missing <- sapply(pop_densities_unique, function(x) length(which(data_info$capture_2==0 & data_info$capture_1==0 & data_info$density_trips==x)))
  truth_zone_counts <- sapply(pop_densities_unique, function(x) length(which(data_info$density_trips==x)))
...
for (a_file in file_names) {
  ...
  fit_1 <- lm(logit_1 ~ data_info$density_trips)
  logit_2 <- log(data_info$p_2/(1 - data_info$p_2))
  fit_2 <- lm(logit_2 ~ data_info$density_trips)
  p_missing <- (1-data_info$p_1)*(1-data_info$p_2)
  logit_missing <- log(p_missing/(1-p_missing))
  fit_missing <- lm(p_missing ~ data_info$density_trips)
  sum1 <- summary(fit_1)
  sum2 <- summary(fit_2)
  summary(fit_missing)
