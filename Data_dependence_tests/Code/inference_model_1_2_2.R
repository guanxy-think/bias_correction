###similar as inference_model_1_2_1.R in "zone_homogenity" but use the optim function instead of spg
...
for (a_file in file_names) {
  ...
  likelihood_function <- function(par) {
    ...
    poisson_upper_bound <- qpois(0.999,par[5:9])
    ...
    for (i in 1:5) {
      likelihoods <- foreach(j = obs_zones_counts[i]:poisson_upper_bound[i], .combine="c", .export=c("obs_zones_counts","obs_1_zones_count","obs_2_zones_count","obs_12_zones_count")) %dopar% {
        ...
        log_likelihood_zones + dpois(j,par[i+4],log=T)
      }
  ...
  estim_optim <- optim(c(-10,0.01,10,-0.01, obs_zones_counts + 100),likelihood_function, method="L-BFGS-B", lower = c(rep(-Inf,4),obs_zones_counts), upper = c(rep(Inf,4),rep(5000,5)), #hessian=T,
  ...
  write.csv(results,paste("G:/My Drive/trip bias paper/scenarios/joint_prob/results_with_density_1/",a_file,".csv",sep=""),row.names=F)
}
