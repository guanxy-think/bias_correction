###similar as inference_model_1_2_5.R in "zone_homogenity", but with 10 zone.
...
for (a_file in file_names) {
  ...
  data_baseline <- read.csv(paste('G:/My Drive/trip bias paper/scenarios/representativeness/more_zones/',a_file,'_regen_with_income.csv',sep=""))
  
  pop_densities_unique <- unique(data_baseline$density_trips_new)
  obs_zones_counts <- sapply(pop_densities_unique, function(x) length(which((data_baseline$capture_1_new==1 | data_baseline$capture_2_new==1) & data_baseline$density_trips_new==x)))
  
  ###For 4-category model
  obs_1_zones_count <- sapply(pop_densities_unique, function(x) length(which((data_baseline$capture_1_new==1 & data_baseline$capture_2_new==0) & data_baseline$density_trips_new==x)))
  obs_2_zones_count <- sapply(pop_densities_unique, function(x) length(which((data_baseline$capture_1_new==0 & data_baseline$capture_2_new==1) & data_baseline$density_trips_new==x)))
  obs_12_zones_count <- sapply(pop_densities_unique, function(x) length(which((data_baseline$capture_1_new==1 & data_baseline$capture_2_new==1) & data_baseline$density_trips_new==x)))
  ...
  ground_truth <- sapply(pop_densities_unique, function(x) length(which(data_baseline$density_trips_new==x)))
  fit_lm <- lm(log(ground_truth) ~ pop_densities_unique)
  fit_1_lm <- glm(data_baseline$capture_1_new~data_baseline$density_trips_new,family=binomial)
  fit_2_lm <- glm(data_baseline$capture_2_new~data_baseline$density_trips_new,family=binomial)
  ...
  likelihood_function <- function(par) {
    ...
    cat(par,file = "C:/Users/guanxy/Documents/logfile_server_5.txt", append=T,fill=T)
    cat(lambda_a,file = "C:/Users/guanxy/Documents/logfile_server_5.txt", append=T,fill=T)
    cat(inf_count,file = "C:/Users/guanxy/Documents/logfile_server_5.txt", append=T,fill=T)
    
    if (max(poisson_upper_bound) > 20000) return(max(poisson_upper_bound))
    if (min(lambda_a) < min(obs_zones_counts)) return(20000)
    ...
    log_likelihood_output <- sum(log(all_likelihoods))
    
    #if (is.infinite(log_likelihood)) log_likelihood <- -100000
    
    cat(log_likelihood_output,file = "C:/Users/guanxy/Documents/logfile_server_5.txt", append=T,fill=T)
  ...
  estim_optim <- optim(c(-4.587858, -2.240e-06, -4.6289060, 0.0001121, 6, 0.0001),likelihood_function, method="L-BFGS-B", lower = c(rep(c(-Inf,-0.01),3)), upper = c(rep(c(Inf,0.01),3)), #hessian=T,
  ...
  write.csv(results,paste("G:/My Drive/trip bias paper/scenarios/representativeness/more_zones/inference_results/",a_file,"_binom_with_income.csv",sep=""),row.names=F)
}
