###similar as inference_model_1_2_3.R in "zone_homogenity", but parameterize lambda_a as function of covariates.
...
for (a_file in file_names) {
  ...
  fit_lm <- lm(log(obs_zones_counts+100) ~ pop_densities_unique)
  
  likelihood_function <- function(par) {
    set.seed(1)
    
    #par[5:9] <- par[5:9]*100000
  ...
  estim_potim <- optim(c(-2.838559e+00,  3.789795e-15, -5.266927e-03,  3.849894e-07, 6, 0.00009),likelihood_function, method="L-BFGS-B", lower = c(rep(-Inf,5),0), upper = c(rep(Inf,5),0.01), #hessian=T,
                       control = list(maxit = 30000,trace = TRUE,ndeps=c(rep(0.00000001,6)),factr=1e5,REPORT=1))
  ...
  write.csv(results,paste("G:/My Drive/trip bias paper/scenarios/representativeness/inference_results/",a_file,".csv",sep=""),row.names=F)
}
