...
for (a_file in file_names) {
  ...
  likelihood_function <- function(par) {
    ...
    cat(par,file = "C:/Users/guanxy/Documents/logfile_server_10.txt", append=T,fill=T)
    cat(lambda_a,file = "C:/Users/guanxy/Documents/logfile_server_10.txt", append=T,fill=T)
    ...
    for (i in 1:10) {
      likelihoods <- foreach(j = obs_zones_counts[i]:poisson_upper_bound[i], .combine="c", .export=c("obs_zones_counts","obs_1_zones_count","obs_2_zones_count","obs_12_zones_count")) %dopar% {
        ...
        log_likelihood_zones <- likelihood_0 + likelihood_1 + likelihood_2 + likelihood_12 + lfactorial(n_a_zones)
    ...
    log_likelihood_output <- sum(all_likelihoods)
    
    #if (is.infinite(log_likelihood)) log_likelihood <- -100000
    
    cat(log_likelihood_output,file = "C:/Users/guanxy/Documents/logfile_server_10.txt", append=T,fill=T)
  ...
  write.csv(results,paste("G:/My Drive/trip bias paper/scenarios/representativeness/more_zones/inference_results/",a_file,"_indiv_model_multinom_no_income_indiv.csv",sep=""),row.names=F)
}
