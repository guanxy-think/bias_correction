...
file_names <- c("scenario_0.1_0.5","scenario_0.1_0.9","scenario_0.5_0.1","scenario_0.5_0.5","scenario_0.5_0.9","scenario_0.9_0.1","scenario_0.9_0.5","scenario_0.9_0.9")
...
for (a_file in file_names) {
  ...
  data_baseline <- read.csv(paste(a_file,'.csv',sep=""))
  ...
  likelihood_function <- function(par) {
    ...
    cat(par,file = "logfile_server.txt", append=T,fill=T)
    all_likelihoods <- c()
    for (i in 1:5) {
    ...
    #cat(num_inf,file = "logfile_server.txt", append=T,fill=T)
    
    log_likelihood <- sum(all_likelihoods)
    
    cat(log_likelihood,file = "logfile_server.txt", append=T,fill=T)
  ...
  #   
  #   write.csv(gibbs_samples,"gibbs_samples_1.csv",row.names = F)
  # }
  # 
  # stopCluster(cl)
  ...
  # estim <- spg(c(1.881069, 0.6850187, -0.2363642, 1.163316, 3.133944, -1.99237, -1.883755, -1.705005, -1.853929, -2.090309, 2853),likelihood_function,lower=c(rep(-Inf,10),max_obs),
  #              control = list(maxit = 300000, trace = TRUE, maxfeval=500000))
  estim_optim <- optim(c(1.881069, 0.6850187, -0.2363642, 1.163316, 3.133944, -1.99237, -1.883755, -1.705005, -1.853929, -2.090309, 2853),likelihood_function, method="L-BFGS-B", lower = c(rep(-Inf,10),max_obs), upper = Inf, hessian=T,
               control = list(maxit = 30000,trace = TRUE,ndeps=c(rep(0.0000000001,10),1),factr=0.45e9,REPORT=1))
  ...
  write.csv(results,paste("results/",a_file,".csv",sep=""),row.names=F)
}
